    ...
    void CreateSegments()
    {
        switch (segCurrentdirection)
        {
            case enDirection.North:
                ...
                {
                    segment.segPrefab = bridgePrefabs[Random.Range(0, bridgePrefabs.Length - 2)];
                }
                else if (segment.segType == enType.L_Corner)
                {
                    segment.segPrefab = bridgePrefabs[bridgePrefabs.Length - 1];
                }
                else if (segment.segType == enType.R_Corner)
                {
                    segment.segPrefab = bridgePrefabs[bridgePrefabs.Length - 2];
                }
                break;
